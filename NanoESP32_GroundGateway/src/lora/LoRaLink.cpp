...
static void resetSpiBus()
{
    // 某些长时间运行 + 较长连线/噪声环境下，SPI 可能进入异常态。
    // 通过 SPI.end()/begin() 重新拉起总线，可显著提高“无需整板 reset 的自愈能力”。
    SPI.end();
    delay(2);
...
static void noteReinit(ReinitReason reason)
{
    ++g_diag.reinit_total;
    g_diag.last_reason = reason;
    g_diag.last_reinit_ms = millis();
    if (reason == ReinitReason::REG_VERSION_BAD) ++g_diag.reinit_regver_bad;
    else if (reason == ReinitReason::TX_TIMEOUT) ++g_diag.reinit_tx_timeout;
    else if (reason == ReinitReason::OP_MODE_BAD) ++g_diag.reinit_opmode_bad;
}

static bool reinit(ReinitReason reason)
{
    // 自愈：RST +（必要时）SPI 总线重置 + 重新写配置。
    // 注意：失败时也不应卡死主循环；由上层继续运行并等待下一次机会。
    noteReinit(reason);
    ...
    g_last_rx_ms = 0;
    g_last_force_rx_ms = millis();
    g_last_health_ms = g_last_force_rx_ms;
    return true;
}

static void healthCheck(uint32_t now)
{
    // 低频健康检查：防止无线/总线进入“看似运行但不再收发”的僵死态。
    if (now - g_last_health_ms < 2000) return;
    g_last_health_ms = now;

    // 诊断采样（不改变射频状态）
    const uint8_t ver0 = readReg(REG_VERSION);
    const uint8_t op0  = readReg(REG_OP_MODE);
    const uint8_t irq0 = readReg(REG_IRQ_FLAGS);
    g_diag.last_regver   = ver0;
    g_diag.last_opmode   = op0;
    g_diag.last_irqflags = irq0;

    // 说明：
    // 之前版本对 IRQ_FLAGS 做“清零校验”会误判：
    // - 在接收过程中（尤其是 ValidHeader/RxDone 窗口）IRQ 会在极短时间内再次置位；
    // - 这在 LoRa 连续接收模式下是正常行为。
    // 误判会导致频繁 reinit，反而造成 RX 漏包 -> 上层 watchdog 反复触发。
    //
    // 因此：健康检查只在出现“强证据”（REG_VERSION 异常/读值前后不一致、或 OpMode 读值异常）时才 reinit。

    // REG_VERSION = 0x00/0xFF 基本可视为 SPI 读异常或射频未上电。
    if (ver0 == 0x00 || ver0 == 0xFF) {
        (void)reinit(ReinitReason::REG_VERSION_BAD);
        return;
    }
    // 再读一次，防止偶发 SPI 抖动（0x12/0x22 正常；但“前后不一致”本身就说明总线不稳定）。
    const uint8_t ver1 = readReg(REG_VERSION);
    if (ver1 != ver0) {
        g_diag.last_regver = ver1;
        (void)reinit(ReinitReason::REG_VERSION_BAD);
        return;
    }

    // OpMode 读出 0x00/0xFF 多半也是 SPI/射频异常。
    if (op0 == 0x00 || op0 == 0xFF) {
        (void)reinit(ReinitReason::OP_MODE_BAD);
        return;
    }
}
...
TxResult sendEx(const uint8_t *payload, size_t len)
{
    ...
    while (true) {
        ...
        if ((millis() - t0) > TX_TIMEOUT_MS) {
            ...
            (void)reinit(ReinitReason::TX_TIMEOUT);
            setOpMode(MODE_RX_CONT);
            g_last_tx_ms = millis();
